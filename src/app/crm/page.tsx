  ...
  return (
    <div>
        ...
        <div className="flex gap-3 overflow-x-auto pb-4 -mx-4 px-4 sm:mx-0 sm:px-0">
          {stages.map((stage) => {
            ...
            return (
              <section
                key={stage}
                className="w-64 shrink-0 rounded-xl bg-slate-100 dark:bg-white/[0.07] border border-slate-200 dark:border-white/10"
              >
                <header className="flex items-center justify-between px-3 py-2.5 border-b border-slate-200 dark:border-white/10">
                  <h2 className="text-xs font-semibold text-slate-700 dark:text-zinc-300 truncate">{stage}</h2>
                  <span className="ml-2 shrink-0 rounded-full bg-slate-200 dark:bg-white/10 px-2 py-0.5 text-[11px] font-semibold text-slate-600 dark:text-zinc-400 tabular-nums">
                    {num(leads.length)}
                  </span>
                </header>
                <div className="p-2 space-y-2">
                  {leads.slice(0, MAX_CARDS).map((lead) => {
                    ...
                    return (
                      <div
                        key={lead.id}
                        className="rounded-lg bg-white dark:bg-[#15121f] border border-slate-200 dark:border-white/10 p-2.5"
                      >
                        <div className="flex items-center gap-1.5">
                          <span
                            className={`h-2 w-2 shrink-0 rounded-full ${STATUS_DOT[lead.status] ?? "bg-slate-300"}`}
                          />
                          <span className="text-xs font-medium text-slate-900 dark:text-zinc-100 truncate">
                  ...
                  {leads.length > MAX_CARDS && (
                    <div className="text-center text-[11px] text-slate-400 dark:text-zinc-500 py-1">
      ...
      <p className="mt-4 text-xs text-slate-400 dark:text-zinc-500">
        Cores: <span className="text-slate-500 dark:text-zinc-400">● frio</span> ·{" "}
