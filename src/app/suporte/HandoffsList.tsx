  ...
  return (
    <div className="space-y-2">
      {rows.map((r) => (
        <div
          key={r.id}
          className="rounded-lg border border-slate-200 p-3 flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"
        >
          <div className="min-w-0">
            ...
            {r.resumo && <p className="mt-1 text-xs text-slate-600 whitespace-pre-wrap">{r.resumo}</p>}
          ...
          {NEXT_STATUS[r.status] && (
            <button
              onClick={() => advance(r)}
              disabled={busy === r.id}
              className="shrink-0 self-start rounded-lg bg-slate-900 px-3 py-1.5 text-xs font-semibold text-white hover:bg-slate-700 disabled:opacity-50"
            >
              {busy === r.id
                ? "…"
                : r.status === "aberto"
