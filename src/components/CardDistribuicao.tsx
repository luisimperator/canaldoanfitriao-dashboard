...
// Card da distribuição do dia 10: o bolo livre, as duas fatias (cofre e
// sócios) e o quanto falta pra meta do cofre. O número grande é o que vai
// pros sócios — é a pergunta que se faz olhando aqui.

function pct(n: number): string {
  return `${Math.round(n * 100)}%`;
}

export function CardDistribuicao({ p }: { p: PoliticaDistribuicao }) {
  ...
  return (
    <div className="mb-4 rounded-xl border border-emerald-200 dark:border-emerald-500/25 bg-emerald-50/60 dark:bg-emerald-500/[0.07] p-5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className="font-mono text-[10px] font-medium uppercase tracking-widest text-emerald-700 dark:text-emerald-400">
            Distribuição de {shortDate(p.dataDistribuicao)}
            {jaPassou && " (já passou)"}
          </div>
