      ...
      {open && (
        <div className="absolute right-0 z-30 mt-2 w-[19rem] max-h-[80vh] overflow-y-auto rounded-xl border border-slate-200 bg-white p-3 shadow-lg">
          ...
          <div className="flex items-center justify-center gap-2 mb-1">
            ...
            <span className="text-xs font-semibold text-slate-700 w-12 text-center">{monthYear}</span>
            <button
              className="rounded px-1.5 text-slate-500 hover:bg-slate-100 disabled:opacity-30"
            ...
            {MESES.map((m, i) => {
              ...
              return (
                <button
                  key={m}
                  disabled={future}
                  className="rounded px-1 py-1 text-[11px] text-slate-600 hover:bg-slate-100 disabled:opacity-30"
                  onClick={() =>
