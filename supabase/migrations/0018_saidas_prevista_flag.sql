-- Saída programada pode ser PREVISÃO (ex.: DDA emitido, ainda não agendado
-- no banco) — entra em amarelo na Provisão até virar agendamento de fato.

alter table provisao_saidas add column if not exists prevista boolean not null default false;

-- provisao_caixa(): inclui o campo prevista em saidas_programadas.
-- (corpo idêntico ao 0017 fora essa linha)
